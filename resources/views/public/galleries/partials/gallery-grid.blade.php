...
<div class="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
    ...
    <a href="{{ route('galleries.show', $gallery->slug) }}" class="group">
        <div class="aspect-w-1 aspect-h-1 xl:aspect-w-7 xl:aspect-h-8 w-full overflow-hidden rounded-lg bg-gray-200 shadow-md">
            <img src="{{ $gallery->cover_image ? $gallery->signed_cover_url : asset('/images/placeholder-gallery.jpg') }}" alt="{{ $gallery->name }}" class="h-full w-full object-cover object-center group-hover:opacity-75 {{ $gallery->cover_image ? 'wasabi-monitored' : '' }}">
        </div>
